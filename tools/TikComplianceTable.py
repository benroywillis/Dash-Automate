...
n10 = json.load( open("/mnt/nobackup-07/Dash/Dash-Corpus/FULLREPORT_"+str(RunID)+"_"+build+".json","r") )
...
for path in n10:
    if path != "Full Report":
        ...
        for BC in n10[path]:
            if BC != "Full Report":
                numbers[refinedPath]["Kernels"] += n10[path][BC]["Total"]["Cartographer Kernels"] 
                numbers[refinedPath]["Tik Kernels"] += n10[path][BC]["Total"]["Tik Kernels"] 
                numbers[refinedPath]["TikSwap Kernels"] += n10[path][BC]["Total"]["Tik Success Kernels"] 
                ...
                numbers[refinedPath]["Traces"] += n10[path][BC]["Total"]["Traces"] 
                numbers[refinedPath]["Successful TikSwap Traces"] += n10[path][BC]["Total"]["Tik Successes"] 
...
for path in n11:
    if path != "FullReport":
        ...
        if numbers.get(refinedPath, None) is None:
            ...
            numbers[refinedPath]["Traces Percent"] = 0
        for BC in n11[path]:
            numbers[refinedPath]["Kernels"] += n11[path][BC]["Total"]["Cartographer Kernels"] 
            numbers[refinedPath]["Tik Kernels"] += n11[path][BC]["Total"]["Tik Kernels"] 
            numbers[refinedPath]["TikSwap Kernels"] += n11[path][BC]["Total"]["TikSwap Kernels"] 
            numbers[refinedPath]["Kernels Percent - Tik"] = ( numbers[refinedPath]["TikSwap Kernels"] / numbers[refinedPath]["Tik Kernels"] * 100 ) if numbers[refinedPath]["Tik Kernels"] > 0 else 0
            numbers[refinedPath]["Kernels Percent - Cartographer"] = ( numbers[refinedPath]["TikSwap Kernels"] / numbers[refinedPath]["Kernels"] * 100 ) if numbers[refinedPath]["Kernels"] > 0 else 0
            numbers[refinedPath]["Traces"] += n11[path][BC]["Traces"]
            numbers[refinedPath]["Successful TikSwap Traces"] += n11[path][BC]["Total"]["Tik Binary Success"] 
            numbers[refinedPath]["Traces Percent"] = ( numbers[refinedPath]["Successful TikSwap Traces"] / numbers[refinedPath]["Traces"] * 100 ) if numbers[refinedPath]["Traces"] > 0 else 0
