...
CC ?= gcc
```
     This is doing two things: first it is defining the buildflow to be whatever your development demands that it be. Second, when the Dash-Ontology environment has been set, the `?=` allows the global environment to override your definition with Dash-Ontology's definition.
   * There are five variables your buildflow must respect
     - CC: C compiler variable 
     - CXX: C++ compiler variable
     - LFLAGS: linker flags necessary for buildflow
     - DASH_ROOT: path to the Dash-Ontology install. This is required when you are including headers for bitcode archives.
     - DASH_DATA: path to the DashData install. This environment variable should be used for file pointers.

   Below is an example of a simple GNU Makefile that respects the Dash-Ontology environment for its compiler, include paths, bitcode archives, and file pointers:
```
CC ?= clang
CXX ?= clang++
...
```
   * There are a few key takeaways here:
     ...
     3. Notice the variables that do not have `?=`. These variables are fixed because they do not come from the Dash-Ontology environment. Only the Dash-Ontology variables need to be overridable.
     4. The bitcode name is representative of the project, not the name of the source code file it came from.

2. File I/O
   * When using file pointers or stdin and stdout to import and export data from your program, DASH_DATA must be used. This is for two reasons:
     ...
     2. Keeping large data files out of Dash-Corpus greatly decreases the repository size and increases is organization.
   * Notice the `CFLAGS` variable defined in the above GNU Makefile example. It is used to define the DASH_DATA path as a compiler macro in your program. This is the recommended method to integrate this environment variable into your source code. Below is an example of how to do this in the source code:
```
// relative DASHDATA path from the root install folder to your project
...
```
   * Here is an example of using $DASH_DATA for runtime args. 
```
./affineTransform.o < $DASH_DATA/opencv/input.jpg > $DASH_DATA/opencv/output.jpg
```